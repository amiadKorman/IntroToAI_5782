...
# This is your HW
"""
מגישים: עמיעד קורמן ובנימין רוזנר
זו היוריסטיקה קבילה, כי המרחק הטוב ביותר הוא סכום המרחקים בשורות ובעמודות.
הוא והיוריסטיקה שלנו במקסימוםאף פעם לא תהיה גדולה מסכום זה, היא קבילה וטובה.

לאחר שנריץ לפי כל אחת מההירסטיקות, נקבל שההירסטיקה שאננו כתבנו 
נותנת ביצועים משמעותית יותר טובים מהיוריסטיקה שהוצגה בכיתה.
זה נובע מכך שאנחנו לא רק אומרים אם יש טעות או לא מבחינת מיקום, אלא כי אנחנו גם מחזירים את המרחק המשוערך.
"""
def hdistance(s):  # the heuristic value of s
    n = math.sqrt(len(s[0]))  # get the size of n in n*n table
    distance = 0
    for i in range(len(s[0])):
        col = abs(s[0].index(i) % n - i % n)  # calculates the distance in the column
        row = abs(s[0].index(i) // n - i // n)  # calculates the distance in the row
        distance += max(col, row)  # pick the max distance
    return distance
